...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_i2i_pixfact_full.log
cd $FT
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] i2i pixfact full $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] i2i pixfact full holdouts done."
