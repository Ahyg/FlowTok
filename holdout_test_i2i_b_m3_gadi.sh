...
CFG=$FT/configs/Sat2Radar-i2i-b-m3-2021summer_gadi.py
WD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_i2i_b_diffusion_2021summer
CKPT=$WD/ckpts/60000.ckpt
OUT=$WD/test_holdout_60000
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$OUT"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m3_test.log
cd $FT
if [ ! -e "$CKPT" ]; then echo "missing $CKPT, abort"; exit 1; fi
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
# M3 uses 500-step DDIM (config.diffusion.sample_steps=500) -> 25x slower than flow arms.
python -u scripts/test_sat2radar_v2v.py \
  ...
  > "$JOBLOG" 2>&1
echo "[$(date '+%F %T')] m3 holdout test done: $OUT/metrics.json"
