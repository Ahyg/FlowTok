...
echo "[$(date '+%F %T')] v2v_cmp_fact_full: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] v2v_cmp_fact_full: after run rc=$RC step=$NEW"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  if [ ! -e "$WD/.holdout_queued" ]; then
    ...
    cd $FT && qsub holdout_test_v2v_cmp_fact_bl128_cond3nan1_full_gadi.sh && echo "queued holdout"
  fi
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_fact_bl128_cond3nan1_full_gadi.sh
else
