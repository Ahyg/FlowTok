...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_i2i_xattn_vpred.log
cd $FT
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] i2i xattn-vpred $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] i2i xattn-vpred full holdouts done."
