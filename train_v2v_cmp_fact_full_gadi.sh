...
echo "[$(date '+%F %T')] v2v cmp fact full: start step=$STEP target=$TARGET (resume auto from ckpt_root)"
...
echo "[$(date '+%F %T')] v2v cmp fact full: after run rc=$RC step=$NEW (124=timeout->resubmit)"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  echo "[$(date '+%F %T')] v2v cmp fact full: progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_fact_full_gadi.sh
else
  echo "[$(date '+%F %T')] v2v cmp fact full: NO progress (step stuck at $NEW, rc=$RC) -> NOT resubmitting"
