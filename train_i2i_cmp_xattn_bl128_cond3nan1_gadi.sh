...
echo "[$(date '+%F %T')] i2i_cmp_xattn_full: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] i2i_cmp_xattn_full: after run rc=$RC step=$NEW"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  if [ ! -e "$WD/.holdout_queued" ]; then
    ...
    cd $FT && qsub holdout_test_i2i_cmp_xattn_bl128_cond3nan1_gadi.sh && echo "queued holdout"
  fi
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_i2i_cmp_xattn_bl128_cond3nan1_gadi.sh
else
