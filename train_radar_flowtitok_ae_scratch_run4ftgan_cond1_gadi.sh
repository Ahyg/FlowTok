...
#PBS -m abe
#PBS -N AE_radar_r4ftgan

# run4ftGAN-cond1 radar AE: 50k LATE-GAN finetune (250k->300k) of run4ft_cond1.
# Resumes from run4ft_cond1/checkpoint-250000 (symlinked into the run4ftgan_cond1 dir);
# auto_resume picks it (global_step=250000) and discriminator_start=250000 engages the
# GAN immediately. Generator LR floor 1e-5; discriminator 1e-4. GAN hyperparams = lab2
# radar sweep winner: discriminator_weight=0.001, disc_factor=1.0, lecam=1e-3.
...
CONFIG_PATH="${FLOWTOK_ROOT}/configs/radar_flowtitok_ae_bl128_vae_scratch_run4ftgan_cond1_gadi.yaml"
...
accelerate launch \
  ...
  --config="${CONFIG_PATH}" \
  > "/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_train_radar_ae_bl128_run4ftgan_cond1.log" 2>&1
