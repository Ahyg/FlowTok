...
def filter_res(sample, min_res=256, min_ratio=0.5, max_ratio=2.0):
    """Global function to filter samples based on resolution and aspect ratio."""
    cfg = sample['json']
    h, w = cfg['original_height'], cfg['original_width']
    ratio = h / w
    longer_side = max(h, w)
    return min_ratio <= ratio <= max_ratio and longer_side >= min_res
...
class PretokenizedWebDataset(SimpleImageDataset):
    def __init__ (
        ...
        train_processing_pipeline = [
            ...
            wds.map(process_recap_text(recap_prob) if process_recap else wds.map(identity)),
