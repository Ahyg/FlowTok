...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_i2i_xattn_s20_s350.log
cd $FT
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] i2i xattn s20 $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] i2i xattn s20 holdouts done."
