...
echo "[$(date '+%F %T')] v2v cmp direct full: start step=$STEP target=$TARGET (resume auto from ckpt_root)"
...
echo "[$(date '+%F %T')] v2v cmp direct full: after run rc=$RC step=$NEW (124=timeout->resubmit)"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_direct_full_gadi.sh
else
  echo "[$(date '+%F %T')] v2v cmp direct full: NO progress (step stuck at $NEW, rc=$RC) -> NOT resubmitting"
