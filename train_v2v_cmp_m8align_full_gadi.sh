...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_m8align_full.log
cd $FT
...
echo "[$(date '+%F %T')] v2v cmp m8align full: start step=$STEP target=$TARGET (resume auto from ckpt_root)"
...
echo "[$(date '+%F %T')] v2v cmp m8align full: after run rc=$RC step=$NEW (124=timeout->resubmit)"

if [ "$NEW" -ge "$TARGET" ]; then
  echo "[$(date '+%F %T')] v2v cmp m8align full: TARGET reached at $NEW. Done."
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub holdout_test_v2v_cmp_m8align_gadi.sh
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "[$(date '+%F %T')] v2v cmp m8align full: progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_m8align_full_gadi.sh
else
  echo "[$(date '+%F %T')] v2v cmp m8align full: NO progress (step stuck at $NEW, rc=$RC) -> NOT resubmitting"
