...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_v2v_m8align_full.log
cd $FT
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] v2v m8align full $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] v2v m8align full holdouts done."
