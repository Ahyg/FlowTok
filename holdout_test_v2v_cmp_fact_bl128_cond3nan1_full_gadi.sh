...
WD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_v2v_cmp_fact_B_bl128_cond3nan1
STEP="${STEP:-800000}"
...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_v2v_fact_full.log
cd $FT
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] v2v fact full $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] v2v fact full holdouts done."
