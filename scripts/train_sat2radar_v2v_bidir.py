...
def build_dataloader(config, mode: str, accelerator: accelerate.Accelerator):
    ...
    # bidir: optional separate val filelist (e.g. val_small pkl)
    if mode == "val":
        filelist_path = config.dataset.get("val_filelist_path", None) or config.dataset.filelist_path
    else:
    ...
    if accelerator.is_main_process:
        ...
        if use_v2v:
            ...
            logging.info(f"  Frame stride: {config.dataset.get('frame_stride', 1)}")
        logging.info(f"  Filelist: {filelist_path}")
        if do_augment:
...
def train(config):
    ...
    def _val_forward_batch(batch):
        ...
        metrics["loss_fwd"] = accelerator.gather(loss.detach()).mean()
        for key, val in loss_dict.items():
            metrics[key] = accelerator.gather(val.detach()).mean()

        # bidir: also compute reverse direction (radar -> sat) loss for monitoring.
        bidir_beta = float(getattr(config, "bidir", {}).get("beta", 0.0))
        if bidir_beta > 0:
            loss_rev, ld_rev = flow_matching_model(
                x=sat_tokens,
                nnet=nnet,
                cond=radar_tokens,
                all_config=config,
            ...
            metrics["loss_rev"] = accelerator.gather(loss_rev.detach()).mean()
            metrics["loss_rev_diff"] = accelerator.gather(ld_rev["diff_loss"].detach()).mean()
        return metrics
    ...
    def train_step(batch):
        ...
        # ── Bidirectional: forward direction (sat → radar) backward ──────────
        bidir_alpha = float(getattr(config, "bidir", {}).get("alpha", 1.0))
        bidir_beta = float(getattr(config, "bidir", {}).get("beta", 0.0))
        accelerator.backward(bidir_alpha * total_loss.mean())
        metrics["loss_fwd"] = accelerator.gather(loss.detach()).mean()

        # ── Bidirectional: reverse direction (radar → sat) ───────────────────
        # Same DiT weights; tokens already encoded so just swap roles.
        # Gradients accumulate on top of forward direction; one optimizer.step()
        # below consumes both.
        if bidir_beta > 0:
            loss_rev, ld_rev = flow_matching_model(
                x=sat_tokens,
            ...
            accelerator.backward(bidir_beta * loss_rev.mean())
            metrics["loss_rev"] = accelerator.gather(loss_rev.detach()).mean()
            metrics["loss_rev_diff"] = accelerator.gather(
                ld_rev["diff_loss"].detach()
            ).mean()
