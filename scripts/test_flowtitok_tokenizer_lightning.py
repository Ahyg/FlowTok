...
import cmweather  # noqa: F401  # just to ensure colormaps are registered if needed
import open_clip
...
def load_py_config(config_path: str):
    ...
    spec = importlib.util.spec_from_file_location("flowtok_config_lightning", config_path)
...
def build_lightning_dataloader(
    data_root: str,
    filelist: str,
    split: str,
    batch_size: int,
    ...
    """
    Build a dataloader over fused npy frames, using only the lightning channel.

    Dataset assumptions (SatelliteRadarNpyDataset docstring):
      - Input array shape: (12, H, W)
      - Channels:
          0-9  : Satellite IR channels
          10   : Lightning
          11   : Radar reflectivity

    Here we configure SatelliteRadarNpyDataset as:
      - mode="satellite"
      - ir_band_indices=[]  (不使用任何 IR 通道)
      - use_lightning=True  (只保留闪电通道)

    在 __getitem__ 中，这会走单帧分支：
      ir = arr[self.ir_band_indices]   -> shape (0, H, W)
      lgt = arr[-2]
      ir_scaled, lgt_scaled = scale_sat_lgt_img(ir, lgt)
      img = concat([ir_scaled, lgt_scaled[None, ...]], axis=0)
    由于 ir_scaled 为空，最终得到的 img 为 (1, H, W)，其中唯一的通道是经过
    [0.1, 50.0] 裁剪并归一化到 [0,1] 的闪电强度。
    """
...
def _save_composite(orig_2d, recon_2d, cmap_name, vmin, vmax, out_path):
    """
    保存三联图：[original | reconstruction | absolute error]，用于闪电重建可视化。
    """
...
def main():
    parser = argparse.ArgumentParser(
        "Test FlowTiTok image tokenizer on fused npy data, using lightning channel (second last) only."
    )
    parser.add_argument(
        "--config",
        required=True,
    ...
    parser.add_argument(
        "--data_root",
        required=True,
        help="Root folder of fused npy data, e.g. /mnt/ssd_1/yghu/Data/71_3m.",
    )
    parser.add_argument(
        "--filelist",
        required=True,
    ...
    parser.add_argument(
        "--fss_thresholds",
        default="",
        help="Comma-separated thresholds for FSS (in lightning physical units). Empty string disables FSS.",
    )
    parser.add_argument(
        "--fss_scales",
        default="",
    ...
    # Build lightning dataloader.
    loader = build_lightning_dataloader(
    ...
    print(f"Tokenizer eval dataset [{args.split.upper()}] (lightning channel, scaled to [0,1]):")
    ...
    # Lightning value range and colormap.
    # 根据 SatelliteRadarNpyDataset.scale_sat_lgt_img 的定义：
    #   l_min, l_max = 0.1, 50.0
    # 这里在可视化时将 [0,1] 反归一化回 [0.1, 50.0]，并使用 'Reds' colormap。
    l_min, l_max = 0.1, 50.0
    ...
    # FSS accumulators on physical lightning fields.
    ...
    for b_idx, batch in enumerate(loader):
        ...
        # FlowTiTok was trained on 3-channel images; replicate lightning channel to 3 channels.
        imgs = imgs_1ch.repeat(1, 3, 1, 1)  # [B, 3, H, W]

        # Build text_guidance.
        if args.use_text:
            # Prompt from filenames: "A lightning density image from <filename>."
            texts = []
            for p in paths:
                fname = os.path.basename(str(p))
                texts.append(f"A lightning density image from {fname}.")
            with torch.no_grad():
                ...
                if text_tokens.shape[2] != text_embed_dim:
                    proj = torch.nn.Linear(text_tokens.shape[2], text_embed_dim, bias=False).to(
                        device
                    )
        ...
        print(
            f"[batch {b_idx+1}/{num_batches}] imgs (lightning) min/max:",
            float(imgs.min()),
            float(imgs.max()),
        )
        ...
        # Use first channel of recon as reconstructed lightning field.
        ...
        if do_images:
            for i in range(imgs_np.shape[0]):
                ...
                # 反归一化到物理范围 [0.1, 50.0] 并用 Reds colormap 可视化。
                orig_ch = imgs_np[i, 0] * (l_max - l_min) + l_min
                recon_ch = recons_np[i, 0] * (l_max - l_min) + l_min
                out_path = os.path.join(
                    args.out_dir, f"{b_idx:03d}_{i:02d}_{safe_name}_lightning.png"
                )
                _save_composite(orig_ch, recon_ch, cmap_lgt, l_min, l_max, out_path)
        ...
        if mse_count > 0 and (b_idx + 1) % 10 == 0:
            ...
            print(
                ...
                f"MSE={batch_mse:.6f}, SSIM={batch_ssim:.6f}"
            )
    ...
    print(f"Saved lightning visualization image(s) to {args.out_dir}")
