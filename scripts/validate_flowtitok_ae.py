...
def main():
    ...
    parser.add_argument("--max_batches_metrics", type=int, default=-1,
                        help="Batches for metrics (-1 = all). FSS/SSIM use this.")
    ...
    for b_idx, batch in enumerate(eval_loader):
        ...
        print("recon:", recon.min().item(), recon.max().item())
        ...
        for i in range(images.shape[0]):
            ...
            if ds_cfg.get("mode", "satellite") == "satellite":
                ...
                for ch in range(10):
                    band = 7 + ch
                    orig_ch = ir[ch]
                    recon_ch = recon_cpu[i, ch] * (ir_max - ir_min) + ir_min
                    ...
                    if do_metrics:
                        ssim_total += ssim(img[ch], recon_cpu[i, ch], data_range=1.0)
                        ssim_count += 1
                        if pysteps_fss_ir:
                            _pysteps_fss_accum(pysteps_fss_ir, recon_cpu[i, ch].astype(np.float64), img[ch].astype(np.float64))

                if ds_cfg.get("use_lightning", True) and img.shape[0] > 10:
                    orig_ch = img[10] * (l_max - l_min) + l_min
                    recon_ch = recon_cpu[i, 10] * (l_max - l_min) + l_min
                    out_path = os.path.join(
                        args.out_dir, f"{b_idx:03d}_{i:02d}_{safe_name}_lightning.png"
                    )
                    _save_composite(orig_ch, recon_ch, cmap_lgt, 0.0, l_max, out_path)
                    if do_metrics:
                        ssim_total += ssim(img[10], recon_cpu[i, 10], data_range=1.0)
                        ssim_count += 1
                        if pysteps_fss_lightning:
                            pred_np = (recon_cpu[i, 10] * (l_max - l_min) + l_min).astype(np.float64)
                            tgt_np = (img[10] * (l_max - l_min) + l_min).astype(np.float64)
                            _pysteps_fss_accum(pysteps_fss_lightning, pred_np, tgt_np)

            else:  # radar
                orig_ch = img[0] * (z_max - z_min) + z_min
                recon_ch = recon_cpu[i, 0] * (z_max - z_min) + z_min
                out_path = os.path.join(
                    args.out_dir, f"{b_idx:03d}_{i:02d}_{safe_name}_radar.png"
                )
                _save_composite(orig_ch, recon_ch, cmap_rad, z_min, z_max, out_path)
                if do_metrics:
                    ssim_total += ssim(img[0], recon_cpu[i, 0], data_range=1.0)
                    ssim_count += 1
                    if pysteps_fss_objects:
                        pred_np = (recon_cpu[i, 0] * (z_max - z_min) + z_min).astype(np.float64)
                        tgt_np = (img[0] * (z_max - z_min) + z_min).astype(np.float64)
