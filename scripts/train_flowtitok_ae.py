...
def main():
    ...
    model, ema_model, loss_module = create_model_and_loss_module(
        config, logger, accelerator, model_type="flowtitok")

    clip_encoder, clip_tokenizer = create_clip_model()

    optimizer, discriminator_optimizer = create_optimizer(config, logger, model, loss_module, model_type="flowtitok")
