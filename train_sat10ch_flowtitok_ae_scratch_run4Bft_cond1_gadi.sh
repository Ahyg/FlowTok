...
#PBS -m abe
#PBS -N AE_s10_r4Bft_train

# run4Bft-cond1 sat10ch AE: 50k continue-finetune (200k->250k) of run4B on the new
# cond1_clip16_p015 dataset. Resumes from checkpoint-200000 (pre-copied into the
# run4Bft_cond1 dir); auto_resume picks the highest checkpoint. LR held at floor 1e-5.
...
CONFIG_PATH="${FLOWTOK_ROOT}/configs/sat10ch_flowtitok_ae_bl128_vae_scratch_run4Bft_cond1_gadi.yaml"
...
accelerate launch \
  ...
  --config="${CONFIG_PATH}" \
  > "/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_train_sat10ch_ae_bl128_run4Bft_cond1.log" 2>&1
