"""
Utility functions for FlowTok training and sampling.

This file was split out from `utils.py` to avoid name conflicts with the
`utils` package directory. All FlowTok training scripts should import from
`flow_utils` instead of `utils`.
"""
import os
import textwrap
...
def set_logger(log_level: str = "info", fname: str | None = None):
...
def get_nnet(name, **kwargs):
    if name == "dimr":
        ...
        from libs.model.flowtok_t2i import FlowTok_S

        return FlowTok_S(kwargs["model_args"])
    elif name == "flowtok-b":
        from libs.model.flowtok_t2i import FlowTok_B

        return FlowTok_B(kwargs["model_args"])
    elif name == "flowtok-l":
        from libs.model.flowtok_t2i import FlowTok_L

        return FlowTok_L(kwargs["model_args"])
    elif name == "flowtok-xl":
        from libs.model.flowtok_t2i import FlowTok_XL

        return FlowTok_XL(kwargs["model_args"])
    elif name == "flowtok-h":
        from libs.model.flowtok_t2i import FlowTok_H

        return FlowTok_H(kwargs["model_args"])
    else:
...
def customized_lr_scheduler(optimizer, warmup_steps: int = -1):
...
def sample2dir(
    ...
    for _batch_size in tqdm(
        ...
        samples, clip_score = sample_fn(
            mini_batch_size,
            return_clipScore=return_clipScore,
            ClipSocre_model=ClipSocre_model,
            config=config,
        )
