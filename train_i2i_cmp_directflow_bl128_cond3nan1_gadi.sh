...
echo "[$(date '+%F %T')] i2i_cmp_directflow: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] i2i_cmp_directflow: after run rc=$RC step=$NEW"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_i2i_cmp_directflow_bl128_cond3nan1_gadi.sh
else
