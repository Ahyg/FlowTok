...
CFG=$FT/configs/Sat2Radar-v2v-cmp-fact-B-bl128-cond3nan1-small20_gadi.py
WD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_v2v_cmp_fact_B_bl128_cond3nan1_small20
TARGET=200000
WALL_SEC=$((23*3600))
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$WD/ckpts"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_fact_s20.log
cd $FT
...
echo "[$(date '+%F %T')] v2v_cmp_fact_s20: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] v2v_cmp_fact_s20: after run rc=$RC step=$NEW"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  if [ ! -e "$WD/.holdout_queued" ]; then
    ...
    cd $FT && qsub holdout_test_v2v_cmp_fact_bl128_cond3nan1_small20_gadi.sh && echo "queued holdout"
  fi
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_fact_bl128_cond3nan1_small20_gadi.sh
else
