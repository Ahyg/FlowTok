...
# Sequence-dim concat of [sat IR tokens | lightning tokens]: 2*77 tokens per frame
# are fed directly into flow matching as x0. No projector, no textVAE, no CFG.
# x_start is radar_tokens concatenated with lgt_tokens (identity on the 2nd half);
# the loss mask zeros out the 2nd half so only sat→radar half contributes.
# At inference we take the first half of the integrated tokens and decode with radar AE.
...
model = Args(
    ...
    cfg_indicator=0.0,           # CFG off — sat+lgt tokens are the only signal.
...
def get_config():
    ...
    # Tiny smoke test: 2k steps, one checkpoint, eval every 500.
    config.train = d(
        n_steps=2_000,
        batch_size=8,
        log_interval=50,
    ...
    # Sequence-dim concat — handled in train_sat2radar_v2v.py.
    ...
    config.workdir = (
        ...
        "sat2radar_flowtok_run_v2v_satlight_seqconcat_tiny"
    )
    config.ckpt_root = config.workdir + "/ckpts"
    config.sample_dir = config.workdir + "/samples"

    # No CFG at inference (scale=1.0).
    config.sample = d(
        sample_steps=20,
        n_samples=16,
        mini_batch_size=4,
