"""M12 cross-attention injection — TINY overfit gate (32-sample, 5k steps).

Same recipe as M12 full but tiny. Reuses the EXACT 32-sample tiny filelist as
M8-tiny (flow_tokconcat_xpred tiny) so the only difference between the two tiny
runs is the injection mechanism (token_concat vs cross_attention).
"""
...
model = Args(
    ...
    cfg_indicator=0.0,
    use_cross_attention=True,   # M12: sat tokens as cross-attention KV.
    noising_type="none",
    noising_scale=0.1,
    textVAE=Args(
        num_blocks=6,
...
def get_config():
    ...
    config.dataset = d(
        # Reuse M8-tiny's exact 32-sample overfit set (controlled ablation).
        filelist_path=(
            "/scratch/kl02/yh0308/Projv2v/Experiments/"
            "sat2radar_flowtok_i2i_b_flow_tokconcat_xpred_2021summer_tiny/dataset_filelist.pkl"
        ),
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_i2i_b_flow_xattn_xpred_2021summer_tiny"
    )
