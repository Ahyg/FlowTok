"""7-arm v2v comparison — Arm 1 (m8block): token_concat + radar_tokens flow.

This is the canonical FlowTok-B / 2021summer template for the 7-arm comparison.
Arm 1 = M8's method (best i2i result) ported to v2v, FlowTok-B, 2021summer set.

  - generation_algorithm = flow_matching
  - flow_prediction_target = radar_tokens  (predict x1 directly)
  - flow_cond_mode = token_concat          (sat tokens prepended in seq-dim)
...
def get_config():
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_cmp_m8block_B"
    )
