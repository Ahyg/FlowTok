"""M11 single-branch textVAE — TINY overfit gate (32-sample, 5k steps).

Same recipe as M2-tiny but with contrastive_loss_weight=0.0 (text-projector
branch OFF). Reuses the EXACT 32-sample tiny filelist as M2-tiny so the only
difference between the two tiny runs is the contrastive branch.
"""
...
model = Args(
    ...
    noising_scale=0.1,
    # Kept True to match M2 architecture; unused while contrastive is off.
...
def get_config():
    ...
    config.dataset = d(
        # Reuse M2-tiny's exact 32-sample overfit set (controlled ablation).
        filelist_path=(
            "/scratch/kl02/yh0308/Projv2v/Experiments/"
            "sat2radar_flowtok_i2i_b_textvae_2021summer_tiny/dataset_filelist.pkl"
        ),
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_i2i_b_textvae_single_2021summer_tiny"
    )
