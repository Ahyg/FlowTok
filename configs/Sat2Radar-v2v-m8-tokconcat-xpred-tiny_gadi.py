"""V2V M8 tiny — overfit gate for token_concat + x1(radar_tokens) flow at v2v scale.

Identical to the full v2v M8 config (XL, 16-frame clips, token_concat + radar_tokens)
EXCEPT:
  - 32-clip overfit filelist (built by scripts/make_overfit_filelist.py)
  - n_steps=12000, frequent eval/save
  - batch_size kept at 8 (== full) ON PURPOSE: this run doubles as the memory
    test for the production batch at the doubled (2*T*L = 2464) DiT sequence.

Success gate (see train_v2v_m8_tokconcat_xpred_tiny_gadi.sh): diff_loss < 0.3 at
the final step and not NaN -> auto-submit the full 200k run.
"""
...
def get_config():
    ...
    config.dataset = d(
        filelist_path=(
            "/scratch/kl02/yh0308/Projv2v/Experiments/"
            "sat2radar_flowtok_v2v_m8_tokconcat_xpred_tiny/dataset_filelist.pkl"
        ),
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_m8_tokconcat_xpred_tiny"
    )
