"""v2v ABLATION 1 "pixfact" — PIXEL space + factorized DiT + flow-matching.

Removes the learned FlowTiTok tokenizer/detokenizer from the hero fact-v2v: a
parameter-free, exactly-invertible pixel patchify (P=8) replaces encode/decode, so
the SAME factorized DiT + flow-matching operates directly on raw radar/sat pixel
patches. Forms the bottom-left cell of the 2x2 paradigm decomposition:
  token+DiT+flow = fact-v2v (hero) ; pixel+DiT+flow = THIS ; pixel+UNet+DDPM = diu-v2v.

P=8 on 128x128: L=(128/8)^2=256 patches/frame, seq=T*L=16*256=4096.
  radar patch dim = 1*8^2 = 64  -> model.channels=64
  sat   patch dim = 11*8^2 = 704 -> model.cond_channels=704 (cross-attn context)
Pixels mapped [0,1]->[-1,1] inside PixelPatchifier (match the N(0,1) flow prior).
Fixed-600k comparison budget. Everything else identical to the fact-B hero.
"""
...
model = Args(
    learn_sigma=False,
    channels=64,            # radar pixel-patch dim = C_radar * P^2 = 1 * 8^2
    cond_channels=704,      # sat pixel-patch dim   = C_sat   * P^2 = 11 * 8^2 (cross-attn KV)
    use_t2i=False,
    clip_dim=16,
    num_clip_token=256,
    num_latent_tokens=256,  # patches/frame L = (128/8)^2; sets factorized n_per_frame + pos
...
def get_config():
    ...
    config.vq_model = d(
        ...
        num_latent_tokens=256,      # patches/frame; drives the sampler seq-reshape assert
        is_legacy=False,
        use_rmsnorm=False,
        use_swiglu=True,
        scale_factor=1.0,
    ...
    config.workdir = (
        ...
        "sat2radar_flowtok_v2v_cmp_pixfact_B_p8_cond3nan1"
    )
