"""7-arm v2v comparison — Arm 3 (direct): sat-tokens-as-x0 velocity prediction.

Arm 3 = the reference sat10ch-direct recipe at FlowTok-B / 2021summer.
Exactly matches Sat2Radar-v2v-sat10ch-direct-FlowTiTok-XL_gadi.py flow settings
(except model size B and 2021summer dataset).

  - generation_algorithm = flow_matching
  - flow_prediction_target = velocity      (default; sat tokens as x0, Gaussian noise as x1)
  - flow_cond_mode = none                  (default; sat tokens are the flow start x0)
  - noising_type = constant                (matches reference)
...
model = Args(
    ...
    noising_type="constant",    # matches direct reference (lightweight Gaussian noise).
    noising_scale=0.1,
    textVAE=Args(
        num_blocks=6,
        hidden_dim=256,
...
def get_config():
    ...
    # ===== Direct method: matches sat10ch-direct reference =====
    config.generation_algorithm = "flow_matching"
    config.flow_prediction_target = "velocity"       # default: velocity target
    config.flow_cond_mode = "none"                   # sat tokens as x0 directly
    ...
    config.dataset = d(
        filelist_path=(
            ...
            "dataset_filelist_v2v_baseline_2021summer.pkl"
        ),
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_cmp_direct_B"
    )
