...
# v2: identical to tiny_seqconcat_nonoise_test.py except the workdir.
# The meaningful change is in scripts/train_sat2radar_v2v.py:
# `_maybe_pad_for_seqconcat` now supervises BOTH fat-frame halves (lgt half
# target velocity ≈ 0 teaches the model to keep lgt stable during inference).
...
def get_config():
    ...
    config.train = d(
        n_steps=5000, batch_size=16, log_interval=50,
        eval_interval=500, save_interval=1000,
        n_samples_eval=4, val_max_batches=4,
    )
    config.optimizer = d(name="adamw", lr=4e-4, weight_decay=0.0, betas=(0.9, 0.95))
    config.lr_scheduler = d(name="customized", warmup_steps=200)

    config.vq_model = d(
        deterministic=False, token_size=16,
        vit_enc_model_size="base", vit_dec_model_size="large",
        vit_enc_patch_size=16, vit_dec_patch_size=16,
        num_latent_tokens=77, is_legacy=False,
        use_rmsnorm=False, use_swiglu=True, scale_factor=1.0,
    )
    ...
    config.dataset = d(
        ...
        v2v=True, num_frames=1, frame_stride=1,
        num_workers_per_gpu=2, crop_size=128,
        ir_band_indices=[0, 2, 6], use_lightning=True,
        augment=d(enabled=False, hflip=False, vflip=False),
    )

    config.workdir = "/scratch/kl02/yh0308/Projv2v/Experiments/tiny_seqconcat_nonoise_v2"
    config.ckpt_root = config.workdir + "/ckpts"
    config.sample_dir = config.workdir + "/samples"

    config.sample = d(
        sample_steps=20, n_samples=4, mini_batch_size=4,
