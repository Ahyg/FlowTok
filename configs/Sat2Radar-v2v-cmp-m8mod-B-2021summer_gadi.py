"""7-arm v2v comparison — Arm 6 (m8mod): token_concat_modality + modality pos-emb.

Arm 6 = M8 variant with modality-aware positional embedding (sat vs radar modality tokens).
Same as m8block except flow_cond_mode = token_concat_modality and use_modality_pos_emb=True.

  - generation_algorithm = flow_matching
  - flow_prediction_target = radar_tokens  (predict x1 directly)
  - flow_cond_mode = token_concat_modality (sat tokens prepended + modality pos-emb)
  - use_modality_pos_emb = True
...
model = Args(
    ...
    use_modality_pos_emb=True,  # Arm 6: modality-aware positional embedding.
    noising_type="none",        # token_concat_modality ignores this (always randn); set
...
def get_config():
    ...
    config.flow_cond_mode = "token_concat_modality"  # sat cond prepended + modality pos-emb
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_cmp_m8mod_B"
    )
