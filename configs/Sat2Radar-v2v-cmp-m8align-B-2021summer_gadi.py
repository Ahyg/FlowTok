"""7-arm v2v comparison — Arm 2 (m8align): token_concat_interleaved + radar_tokens flow.

Arm 2 = M8 variant with interleaved sat/radar token layout (alternating frames).
Same as m8block except flow_cond_mode = token_concat_interleaved.

  - generation_algorithm = flow_matching
  - flow_prediction_target = radar_tokens  (predict x1 directly)
  - flow_cond_mode = token_concat_interleaved  (sat and radar frames alternated in seq-dim)
...
def get_config():
    ...
    config.flow_cond_mode = "token_concat_interleaved"  # sat/radar frames alternated in seq-dim
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_cmp_m8align_B"
    )
