"""Capacity-scaling of Arm 7 (xattn) — XL backbone.

Identical to Sat2Radar-v2v-cmp-xattn-B-2021summer_gadi.py EXCEPT the DiT size
is flowtok-XL (depth28/hidden1152, ~674M) instead of flowtok-B (~130M). Same
arm7 cross-attention conditioning, same 1445-clip 2021summer set, same 60k
steps / bs=8 — so this is a clean "only model size changed" ablation vs the
finished arm7-B (holdout wFSS 0.242).

  - flow_cond_mode = cross_attention, use_cross_attention = True
  - flow_prediction_target = radar_tokens (predict x1)
  - model size = flowtok-XL
  - dataset = 2021summer small set (1445 train clips), 60k steps
"""
...
def get_config():
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_cmp_xattn_XL"
    )
