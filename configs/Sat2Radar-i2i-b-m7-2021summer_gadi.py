"""M7 — M5's token_concat conditioning, but flow-matching (velocity) algorithm.

Same data + AE + DiT-B + 60k as M1; only conditioning mechanism changes:
  M1: sat tokens = flow x0; DiT sees x_t (shape [B,L,C]) at every step
  M7: noise = randn (decoupled from sat); cond=sat is prepended in seq-dim at
      every DiT call -> inp shape [B,2L,C]; output's last L tokens are the
      velocity prediction. Pos embed is built on-the-fly per seq_len, so no
      DiT code change is needed (same trick as M5).
"""
...
model = Args(
    ...
    cfg_indicator=0.0,
    # Under token_concat the noising_type code path is skipped (cond enters
    # via concat, not via the flow x0), so these are dead values kept here
    # only for argparse/Args compatibility.
    noising_type="none",
    noising_scale=0.1,
    textVAE=Args(
        num_blocks=6,
...
def get_config():
    ...
    # M7: flow matching (velocity), token_concat conditioning.
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_i2i_b_flow_tokconcat_2021summer"
    )
