"""V2V M8-aligned tiny — overfit gate for the INTERLEAVED token_concat + x1 arm.

Identical to the full m8align config (XL, 16f, token_concat_interleaved) except the
32-clip overfit filelist, n_steps=12000, frequent eval/save, bs kept at 8 (== full,
so it doubles as the seq-2464 memory test). Same 32 clips as the block-M8 tiny, so
the two arms' tiny runs are directly comparable.
"""
...
def get_config():
    ...
    config.dataset = d(
        filelist_path=(
            "/scratch/kl02/yh0308/Projv2v/Experiments/"
            "sat2radar_flowtok_v2v_m8align_tokconcat_xpred_tiny/dataset_filelist.pkl"
        ),
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_v2v_m8align_tokconcat_xpred_tiny"
    )
