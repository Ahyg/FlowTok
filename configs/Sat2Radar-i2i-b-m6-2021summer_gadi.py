"""M6 — M2 variant: lower KLD to AE level + disable extra noising.

Two-knob tweak vs M2:
  - `kld_loss_weight`:  1e-4  ->  1e-6   (match the sat/radar AE's kl_weight,
                                          avoid double-Gaussianization)
  - `noising_type`:    'constant' -> 'none'  (FlowTok-XL original t2i setting;
                                              stop perturbing x0 with extra noise)
All other knobs (contrastive_loss_weight=1.0, textVAE.hidden_dim=256,
num_attention_heads=4, etc.) are kept identical to M2 for clean ablation.
"""
...
model = Args(
    ...
    cfg_indicator=0.0,
    noising_type="none",        # M6: was "constant" in M2; align with FlowTok t2i.
    noising_scale=0.1,          # unused when noising_type='none'
...
def get_config():
    ...
    # M6 change: kld_loss_weight 1e-4 -> 1e-6 (match AE kl_weight).
    config.losses = d(
    ...
    config.workdir = (
        "/scratch/kl02/yh0308/Projv2v/Experiments/"
        "sat2radar_flowtok_i2i_b_textvae_lowkld_2021summer"
    )
