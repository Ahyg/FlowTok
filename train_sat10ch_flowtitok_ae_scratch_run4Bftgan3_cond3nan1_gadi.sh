...
#PBS -m abe
#PBS -N AE_sat_r4Bftgan3

# run4BftGAN3-cond3nan1 sat10ch AE: CONTINUE GAN finetune 350k->400k, SAME recipe as
# run4Bftgan2 (pure step extension; no code/loss/arch change). Resumes from
# run4Bftgan2_cond3nan1/checkpoint-350000 (symlinked into the run4Bftgan3 dir); auto_resume
# picks it (global_step=350000) and discriminator_start=300000 keeps GAN engaged.
# Generator LR floor 1e-5; discriminator 1e-4. disc_weight=0.001. reconstruction_loss=l2.
# Motivation: holdout lightning-FSS gains were monotone & not yet plateaued at 350k; this
# segment tests whether they keep climbing. save_every=50000 => only checkpoint-400000 saved.
...
CONFIG_PATH="${FLOWTOK_ROOT}/configs/sat10ch_flowtitok_ae_bl128_vae_scratch_run4Bftgan3_cond3nan1_gadi.yaml"
...
accelerate launch \
  ...
  --config="${CONFIG_PATH}" \
  > "/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_train_sat10ch_ae_bl128_run4Bftgan3_cond3nan1.log" 2>&1
