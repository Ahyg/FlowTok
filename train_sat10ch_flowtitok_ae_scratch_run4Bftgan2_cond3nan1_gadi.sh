...
#PBS -m abe
#PBS -N AE_sat_r4Bftgan2

# run4BftGAN2-cond3nan1 sat10ch AE: CONTINUE GAN finetune 300k->350k on the new 6-year
# "71full" dataset (cond3nan1 full i2i train, 71,040 frames). Resumes from
# run4Bftgan_cond1/checkpoint-300000 (symlinked into the run4Bftgan2 dir); auto_resume
...
CONFIG_PATH="${FLOWTOK_ROOT}/configs/sat10ch_flowtitok_ae_bl128_vae_scratch_run4Bftgan2_cond3nan1_gadi.yaml"
...
accelerate launch \
  ...
  --config="${CONFIG_PATH}" \
  > "/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_train_sat10ch_ae_bl128_run4Bftgan2_cond3nan1.log" 2>&1
