...
OUT=$WD/test_holdout_60000_nfe100
NFE=100
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$OUT"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m3_nfe${NFE}.log
cd $FT
...
python -u scripts/test_sat2radar_v2v.py \
  ...
  > "$JOBLOG" 2>&1
echo "[$(date '+%F %T')] m3 nfe=$NFE holdout done: $OUT/metrics.json"
