...
TEST_PKL_CT=/g/data/kl02/yh0308/Data/71/filelists/dataset_filelist_v2v_test_202407_202507_cond3nan1_clip16_p005_seed42.pkl
...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_v2v_factddpm_full.log
cd $FT
if [ ! -e "$CKPT" ]; then echo "missing $CKPT, abort"; exit 1; fi
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
# generation_algorithm=diffusion in the config -> test_sat2radar_v2v.py uses DDIM-500 automatically.
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] v2v factddpm full $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] v2v factddpm full holdouts done."
