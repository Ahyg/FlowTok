...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m8_full.log
cd $FT
LATEST=$(ls -t "$WD"/ckpts/*.ckpt 2>/dev/null | head -1)
STEP=$(echo "$LATEST" | grep -oP '[0-9]+(?=\.ckpt)'); STEP=${STEP:-0}
echo "[$(date '+%F %T')] m8 full: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] m8 full: after run step=$NEW"
if [ "$NEW" -lt "$TARGET" ]; then
  cd $FT && qsub train_i2i_b_m8_full_gadi.sh
else
  cd $FT && qsub holdout_test_i2i_b_m8_gadi.sh && echo "[$(date '+%F %T')] m8 hit target, queued holdout test"
fi
