...
#PBS -m abe
#PBS -N AE_radar_r4hilgan

# run4hilGAN-cond3nan1 radar AE: restart GAN finetune 300k->350k from the HEALTHY 300k,
# with a HILBURN precipitation-weighted recon (exp(5*y^3)*0.5*err^2) so reconstruction
# prioritizes high-dBZ cores, + anti-drift guardrail (discriminator_learning_rate 1e-4->3e-5;
# force_lr_floor_on_resume now pins BOTH gen=1e-5 and disc=3e-5). Resumes from
# run4ftgan_cond1/checkpoint-300000 (symlinked into the run4hilgan dir); auto_resume picks it
# (global_step=300000); discriminator_start=300000 keeps GAN engaged. Dataset FIXED = cond3nan1
# (71full). save_every=50000 => only the final checkpoint-350000 is written.
...
CONFIG_PATH="${FLOWTOK_ROOT}/configs/radar_flowtitok_ae_bl128_vae_scratch_run4hilgan_cond3nan1_gadi.yaml"
...
accelerate launch \
  ...
  --config="${CONFIG_PATH}" \
  > "/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_train_radar_ae_bl128_run4hilgan_cond3nan1.log" 2>&1
