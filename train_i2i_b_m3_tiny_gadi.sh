...
CFG=$FT/configs/Sat2Radar-i2i-b-m3-tiny_gadi.py
TD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_i2i_b_diffusion_2021summer_tiny
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$TD"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m3_tiny.log
cd $FT
echo "[$(date '+%F %T')] m3 tiny: start"
...
echo "[$(date '+%F %T')] m3 tiny: final diff_loss=$LASTLOSS"
if [ -n "$LASTLOSS" ] && awk -v v="$LASTLOSS" 'BEGIN{exit !(v+0<0.02)}'; then
  echo "GATE PASS -> submitting m3 full"
  rm -f "$TD/TINY_FAIL"
  cd $FT && qsub train_i2i_b_m3_full_gadi.sh
else
  echo "GATE FAIL (loss=$LASTLOSS) -> m3 full NOT submitted"
