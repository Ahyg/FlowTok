...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_cmp_m8align_s20.log
cd $FT
...
echo "[$(date '+%F %T')] i2i_cmp_m8align_s20: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] i2i_cmp_m8align_s20: after run rc=$RC step=$NEW"
...
if [ "$NEW" -ge "$TARGET" ]; then
  ...
  if [ ! -e "$WD/.holdout_queued" ]; then
    ...
    cd $FT && qsub holdout_test_i2i_cmp_m8align_bl128_cond3nan1_small20_gadi.sh && echo "queued holdout"
  fi
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_i2i_cmp_m8align_bl128_cond3nan1_small20_gadi.sh
else
