...
CFG=$FT/configs/Sat2Radar-v2v-cmp-xattn-tiny_gadi.py
TD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_v2v_cmp_xattn_tiny
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$TD"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_xattn_tiny.log
cd $FT
echo "[$(date '+%F %T')] v2v cmp xattn tiny: start (XL, token_concat_INTERLEAVED+x1, 16f, bs=8, fat-frame=2L=154, seq=2464)"
...
echo "[$(date '+%F %T')] v2v cmp xattn tiny: final step=$LASTSTEP diff_loss=$LASTLOSS"
PASS=0
...
if [ "$PASS" = "1" ]; then
  echo "GATE PASS -> submitting v2v cmp xattn full"
  rm -f "$TD/TINY_FAIL"
  cd $FT && qsub train_v2v_cmp_xattn_full_gadi.sh
else
  echo "GATE FAIL (step=$LASTSTEP loss=$LASTLOSS) -> v2v cmp xattn full NOT submitted"
