...
CFG=$FT/configs/Sat2Radar-v2v-cmp-m8block-B-2021summer_gadi.py
WD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_v2v_cmp_m8block_B
TARGET=200000
WALL_SEC=$((47*3600))
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$WD/ckpts"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_m8block_full.log
cd $FT
...
echo "[$(date '+%F %T')] v2v cmp m8block full: start step=$STEP target=$TARGET (resume auto from ckpt_root)"
...
echo "[$(date '+%F %T')] v2v cmp m8block full: after run rc=$RC step=$NEW (124=timeout->resubmit)"

if [ "$NEW" -ge "$TARGET" ]; then
  echo "[$(date '+%F %T')] v2v cmp m8block full: TARGET reached at $NEW. Done."
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub holdout_test_v2v_cmp_m8block_gadi.sh
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "[$(date '+%F %T')] v2v cmp m8block full: progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_m8block_full_gadi.sh
else
  echo "[$(date '+%F %T')] v2v cmp m8block full: NO progress (step stuck at $NEW, rc=$RC) -> NOT resubmitting"
