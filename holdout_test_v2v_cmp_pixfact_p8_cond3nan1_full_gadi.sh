...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_htest_v2v_pixfact_full.log
cd $FT
...
run_test () {
  ...
  mkdir -p "$OUT"
  echo "[$(date '+%F %T')] v2v pixfact full $TAG -> $OUT" >> "$JOBLOG"
...
echo "[$(date '+%F %T')] v2v pixfact full holdouts done."
