...
CFG=$FT/configs/Sat2Radar-v2v-cmp-satenc-tiny_gadi.py
TD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_v2v_cmp_satenc_tiny
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$TD"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_satenc_tiny.log
cd $FT
echo "[$(date '+%F %T')] v2v cmp satenc tiny: start (B, cross_attn + sat self-attn encoder N=6, radar x1, 16f, bs=8, radar seq=1232)"
...
echo "[$(date '+%F %T')] v2v cmp satenc tiny: final step=$LASTSTEP diff_loss=$LASTLOSS"
PASS=0
...
if [ "$PASS" = "1" ]; then
  echo "GATE PASS -> submitting v2v cmp satenc full"
  rm -f "$TD/TINY_FAIL"
  cd $FT && qsub train_v2v_cmp_satenc_full_gadi.sh
else
  echo "GATE FAIL (step=$LASTSTEP loss=$LASTLOSS) -> v2v cmp satenc full NOT submitted"
