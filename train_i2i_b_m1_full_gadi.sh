...
CFG=$FT/configs/Sat2Radar-i2i-b-m1-2021summer_gadi.py
WD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_i2i_b_baseline_2021summer
TARGET=60000
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m1_full.log
cd $FT
LATEST=$(ls -t "$WD"/ckpts/*.ckpt 2>/dev/null | head -1)
STEP=$(echo "$LATEST" | grep -oP '[0-9]+(?=\.ckpt)'); STEP=${STEP:-0}
echo "[$(date '+%F %T')] m1 full: start step=$STEP target=$TARGET"
...
echo "[$(date '+%F %T')] m1 full: after run step=$NEW"
if [ "$NEW" -lt "$TARGET" ]; then
  cd $FT && qsub train_i2i_b_m1_full_gadi.sh
else
  cd $FT && qsub holdout_test_i2i_b_m1_gadi.sh && echo "[$(date '+%F %T')] m1 hit target, queued holdout test"
fi
