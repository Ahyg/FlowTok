...
# Match the M1-M8 gen-metrics dir so the metrics.json is directly comparable.
OUT=$WD/test_holdout_60000_genmetrics
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$OUT"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m12_test.log
cd $FT
...
python -u scripts/test_sat2radar_v2v.py \
  ...
  > "$JOBLOG" 2>&1
echo "[$(date '+%F %T')] m12 holdout test done (FSS+FID/sFID/KID): $OUT/metrics.json"
