...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m8_v2vali.log
cd $FT
...
python -u scripts/test_sat2radar_v2v.py \
  ...
  > "$JOBLOG" 2>&1
echo "[$(date '+%F %T')] m8 v2v-aligned holdout done: $OUT/metrics.json"
