...
CFG=$FT/configs/Sat2Radar-i2i-b-m5-tiny_gadi.py
TD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_i2i_b_diffusion_tokconcat_2021summer_tiny
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$TD"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m5_tiny.log
cd $FT
echo "[$(date '+%F %T')] m5 tiny: start"
accelerate launch --num_processes 1 scripts/train_sat2radar_v2v.py --config="$CFG" > "$JOBLOG" 2>&1 || true
# M3/M5 diffusion arms can't hit the 0.02 flow gate (random t sampling -> loss floor ~0.15-0.20).
# Calibrated gate per Diffi2i-style 5k tiny: pass if final diff_loss < 0.3 AND step >= 4900.
...
echo "[$(date '+%F %T')] m5 tiny: final step=$LASTSTEP diff_loss=$LASTLOSS"
PASS=0
...
if [ "$PASS" = "1" ]; then
  echo "GATE PASS -> submitting m5 full"
  rm -f "$TD/TINY_FAIL"
  cd $FT && qsub train_i2i_b_m5_full_gadi.sh
else
  echo "GATE FAIL (step=$LASTSTEP loss=$LASTLOSS) -> m5 full NOT submitted"
