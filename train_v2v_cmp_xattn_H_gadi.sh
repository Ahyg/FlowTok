...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_xattn_H.log
cd $FT
...
echo "[$(date '+%F %T')] v2v cmp xattn H: start step=$STEP target=$TARGET (resume auto from ckpt_root)"
...
echo "[$(date '+%F %T')] v2v cmp xattn H: after run rc=$RC step=$NEW (124=timeout->resubmit)"

if [ "$NEW" -ge "$TARGET" ]; then
  echo "[$(date '+%F %T')] v2v cmp xattn H: TARGET reached at $NEW. Done."
  rm -f "$WD/RESUBMIT_STALLED"
  echo "[$(date '+%F %T')] auto-submitting holdout test for xattn H"
  cd $FT && qsub holdout_test_v2v_cmp_xattn_H_gadi.sh
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "[$(date '+%F %T')] v2v cmp xattn H: progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_xattn_H_gadi.sh
else
  echo "[$(date '+%F %T')] v2v cmp xattn H: NO progress (step stuck at $NEW, rc=$RC) -> NOT resubmitting"
