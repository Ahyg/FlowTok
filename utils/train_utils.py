    ...
    if "config" in cli_conf:
        ...
        for i in range(1, len(argv)):
            ...
            if argv[i] == "--config" and i + 1 < len(argv):
                ...
                break
    if not config_path:
        raise ValueError("Please provide --config path/to/config.yaml")
...
def _check_finite_posteriors(posteriors, accelerator, logger):
    ...
    global_flag = accelerator.gather(local_flag).max().item()
    if global_flag > 0 and accelerator.is_main_process:
        mean_stats = _finite_stats(posteriors.mean.detach())
        logvar_stats = _finite_stats(posteriors.logvar.detach())
        logger.error(
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if model_type == "flowtitok":
        model_config = _build_flowtitok_config(config)
    else:
        model_config = config
    ...
    if config.training.use_ema:
        ...
        def load_model_hook(models, input_dir):
            ...
            if not os.path.isdir(ema_dir):
                logger.warning(f"EMA model not found at {ema_dir}. Skipping EMA load.")
                return
            ema_ckpt = os.path.join(ema_dir, "pytorch_model.bin")
            if not os.path.exists(ema_ckpt):
                logger.warning(f"EMA checkpoint missing {ema_ckpt}. Skipping EMA load.")
                return
    ...
    if accelerator.is_main_process:
        if model_type in ["titok"]:
            ...
            col_names=("input_size", "output_size", "num_params", "params_percent", "kernel_size", "mult_adds"))
            logger.info(model_summary_str)
        elif model_type in ["flowtitok"]:
            in_channels = config.model.vq_model.get("in_channels", 3)
            input_image_size = (1, in_channels, config.dataset.preprocessing.crop_size, config.dataset.preprocessing.crop_size)
            input_text_size = (
                1,
...
def create_dataloader(config, logger, accelerator):
    ...
    if dataset_config.get("filelist_path"):
        train_dataset = SatelliteRadarNpyDataset(
            ...
            mode=dataset_config.get("mode", "satellite"),
            ir_band_indices=dataset_config.get("ir_band_indices"),
    ...
    if dataset_config.get("pretokenization", "") and dataset_config.get("dataset_with_text_label", False) is True:
        from data.webdataset_reader import PretokenizedWebDataset
    ...
    # SimpleImageDataset
        ...
        from data.webdataset_reader import SimpleImageDataset
    ...
    # potentially, use a pretokenized dataset for ImageNet speed-up.
        ...
        if dataset_config.get("pretokenization", ""):
            from data.webdataset_reader import PretoeknizedDataSetJSONL
            train_dataloader = DataLoader(
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.generate_every == 0 and accelerator.is_main_process:
                ...
                reconstruct_images(
                    ...
                    text_guidance=text_guidance[:config.training.num_generated_images] if model_type in ["tatitok", "flowtitok"] else None,
        ...
        if model_type == "tatitok":
            try:
                ...
                raise ModuleNotFoundError(
                    "imagenet_classes is required for tatitok eval with class_id."
                ) from exc
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    if config is not None:
        mode = config.dataset.params.get("mode")
        use_lightning = config.dataset.params.get("use_lightning", False)
    ...
    for i in range(original_images.shape[0]):
        ...
        safe_name = os.path.basename(raw_name).replace(os.sep, "_").replace(":", "_")
...
def load_checkpoint(checkpoint_path: Path, accelerator, logger, strict=True):
    ...
    if not metadata_file.exists():
        logger.warning(f"Checkpoint missing {metadata_file}. Skipping resume.")
        return 0
