...
#PBS -m abe
#PBS -N AE_radar_r4ft_train

# run4ft-cond1 radar AE: 50k continue-finetune (200k->250k) of run4 on the new
# cond1_clip16_p015 dataset. Resumes from checkpoint-200000 (pre-copied into the
# run4ft_cond1 dir); auto_resume picks the highest checkpoint. LR held at floor 1e-5.
...
CONFIG_PATH="${FLOWTOK_ROOT}/configs/radar_flowtitok_ae_bl128_vae_scratch_run4ft_cond1_gadi.yaml"
...
accelerate launch \
  ...
  --config="${CONFIG_PATH}" \
  > "/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_train_radar_ae_bl128_run4ft_cond1.log" 2>&1
