...
# Test FlowTiTok image tokenizer reconstruction on fused satellite npy data
# (e.g. /mnt/ssd_1/yghu/Data/71_3m), using IR channels 0/2/6 (7, 9, 13) as
# pseudo-RGB after normalization.
#
# Assumes you already have:
#   - Satellite dataset at: /mnt/ssd_1/yghu/Data/71_3m
...
#
# Usage:
#   bash run_flowtitok_satellite_recon.sh
#
# Optional overrides:
#   CUDA_VISIBLE_DEVICES=0 bash run_flowtitok_satellite_recon.sh
#   DATA_ROOT=/path/to/71_3m CKPT_PATH=/path/to/FlowTiTok_512.bin OUT_DIR=/path/to/out bash run_flowtitok_satellite_recon.sh
...
OUT_DIR="${OUT_DIR:-/mnt/ssd_1/yghu/Experiments/flowtok_tokenizer_satellite71_recon}"
SPLIT="${SPLIT:-val}"
...
# FSS settings on grayscale (normalized [0,1]); can be overridden.
...
CMD=(python "${ROOT_DIR}/scripts/test_flowtitok_tokenizer_satellite.py"
  --config "${CONFIG_PATH}"
  --ckpt "${CKPT_PATH}"
  --data_root "${DATA_ROOT}"
  --filelist "${FILELIST}"
...
echo "[DONE] Satellite reconstruction metrics and images saved to: ${OUT_DIR}"
