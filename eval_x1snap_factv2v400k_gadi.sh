...
#PBS -m abe
#PBS -N x1snap_fct400
# Full-set snap inference on the CURRENT-BEST fact-v2v @400k ckpt.
# x1_snap_t=0.99 = at the last step (t->1) output the model's clean-radar x1_hat directly,
# skipping the remaining Euler integration. Replicates the production holdout invocation EXACTLY
# (max_batches_metrics=-1, gen metrics ON, same nofilt filelist/batch) and differs ONLY in --x1_snap_t.
# Compares against the existing test_holdout_400000_nofilt baseline (avg_fss 0.4611).
set -uo pipefail
...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_x1snap_factv2v400k.log
exec > "$JOBLOG" 2>&1
cd $FT
[ -e "$CKPT" ] || { echo "missing $CKPT"; exit 1; }

echo "[$(date '+%F %T')] fact-v2v @${STEP} snap099 full-set -> $SNAP_DIR  (baseline=$BASE_DIR)"
...
echo "[$(date '+%F %T')] === COMPARE snap099 vs baseline (fact-v2v @${STEP}) ==="
...
PY
echo "[$(date '+%F %T')] DONE fact-v2v @${STEP}"
