...
CFG=$FT/configs/Sat2Radar-v2v-cmp-xattn-XL-2021summer_gadi.py
...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_v2v_cmp_xattn_XL.log
cd $FT
...
echo "[$(date '+%F %T')] v2v cmp xattn XL: start step=$STEP target=$TARGET (resume auto from ckpt_root)"
...
echo "[$(date '+%F %T')] v2v cmp xattn XL: after run rc=$RC step=$NEW (124=timeout->resubmit)"

if [ "$NEW" -ge "$TARGET" ]; then
  echo "[$(date '+%F %T')] v2v cmp xattn XL: TARGET reached at $NEW. Done."
  rm -f "$WD/RESUBMIT_STALLED"
  echo "[$(date '+%F %T')] auto-submitting holdout test for xattn XL"
  cd $FT && qsub holdout_test_v2v_cmp_xattn_XL_gadi.sh
elif [ "$RC" = "124" ] || [ "$NEW" -gt "$STEP" ]; then
  echo "[$(date '+%F %T')] v2v cmp xattn XL: progress $STEP->$NEW (rc=$RC), re-qsub"
  rm -f "$WD/RESUBMIT_STALLED"
  cd $FT && qsub train_v2v_cmp_xattn_XL_gadi.sh
else
  echo "[$(date '+%F %T')] v2v cmp xattn XL: NO progress (step stuck at $NEW, rc=$RC) -> NOT resubmitting"
