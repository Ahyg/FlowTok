...
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m5_test.log
cd $FT
if [ ! -e "$CKPT" ]; then echo "missing $CKPT, abort"; exit 1; fi
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
# M5 = M3 variant: 500-step DDIM, token-concat conditioning -> roughly same wallclock as M3.
python -u scripts/test_sat2radar_v2v.py \
  ...
  > "$JOBLOG" 2>&1
echo "[$(date '+%F %T')] m5 holdout test done: $OUT/metrics.json"
