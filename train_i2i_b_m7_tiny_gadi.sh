...
CFG=$FT/configs/Sat2Radar-i2i-b-m7-tiny_gadi.py
TD=/scratch/kl02/yh0308/Projv2v/Experiments/sat2radar_flowtok_i2i_b_flow_tokconcat_2021summer_tiny
mkdir -p /scratch/kl02/$USER/Projv2v/job_logs "$TD"
JOBLOG=/scratch/kl02/$USER/Projv2v/job_logs/${PBS_JOBID}_i2i_b_m7_tiny.log
cd $FT
echo "[$(date '+%F %T')] m7 tiny: start"
accelerate launch --num_processes 1 scripts/train_sat2radar_v2v.py --config="$CFG" > "$JOBLOG" 2>&1 || true
# Flow + token_concat (velocity target): scale similar to M1/M4. Use lenient
# gate diff_loss<0.3 AND step>=4900 AND not NaN.
...
echo "[$(date '+%F %T')] m7 tiny: final step=$LASTSTEP diff_loss=$LASTLOSS"
PASS=0
...
if [ "$PASS" = "1" ]; then
  echo "GATE PASS -> submitting m7 full"
  rm -f "$TD/TINY_FAIL"
  cd $FT && qsub train_i2i_b_m7_full_gadi.sh
else
  echo "GATE FAIL (step=$LASTSTEP loss=$LASTLOSS) -> m7 full NOT submitted"
