...
class ReconstructionLoss_Stage2(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        loss_config = config.losses
        disc_in_channels = getattr(config.model.vq_model, "out_channels", None)
        if disc_in_channels is None:
            disc_in_channels = getattr(config.model.vq_model, "in_channels", 3)
        self.discriminator = NLayerDiscriminator(num_channels=int(disc_in_channels))
    ...
    def _forward_generator(self,
        ...
        if self.perceptual_weight > 0:
            if str(self.perceptual_loss_type).lower().startswith("ssim"):
                perceptual_loss = self.ssim_loss(inputs, reconstructions).mean()
            else:
